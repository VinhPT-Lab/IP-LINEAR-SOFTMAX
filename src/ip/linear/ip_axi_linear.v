...
`timescale 1 ns / 1 ps
	...
	// Add user internal logic
	logic start_attn_score;
	logic attn_score_done;
	logic busy;
	
// Instantiation of Axi Bus Interface S00_AXI
	ip_axi_linear_slave_lite_v5_0_S00_AXI # ( 
		.C_S_AXI_DATA_WIDTH(C_S00_AXI_DATA_WIDTH),
		.C_S_AXI_ADDR_WIDTH(C_S00_AXI_ADDR_WIDTH),
		.D_MODEL            (D_MODEL),             
		.SEQ_LEN            (SEQ_LEN),              
		.DATA_WIDTH         (DATA_WIDTH)          
	) ip_axi_linear_slave_lite_v5_0_S00_AXI_inst (
		...
		.o_start_attn_score (start_attn_score),     
		.i_attn_score_done  (attn_score_done),
		.i_busy(busy)         
	);
